...
const STEPS = [
  ...
  { id: 'entorno',    label: 'Entorno',  short: '02' },
  { id: 'evaluacion', label: 'Evaluación', short: '03' },
  { id: 'diagrama',   label: 'Diagrama', short: '04' },
  { id: 'reporte',    label: 'Reporte',  short: '05' },
]

function SunIcon()  { return <svg viewBox="0 0 20 20" fill="currentColor" className="w-4 h-4"><path fillRule="evenodd" d="M10 2a1 1 0 011 1v1a1 1 0 11-2 0V3a1 1 0 011-1zm4 8a4 4 0 11-8 0 4 4 0 018 0zm-.464 4.95l.707.707a1 1 0 001.414-1.414l-.707-.707a1 1 0 00-1.414 1.414zm2.12-10.607a1 1 0 010 1.414l-.706.707a1 1 0 11-1.414-1.414l.707-.707a1 1 0 011.414 0zM17 11a1 1 0 100-2h-1a1 1 0 100 2h1zm-7 4a1 1 0 011 1v1a1 1 0 11-2 0v-1a1 1 0 011-1zM5.05 6.464A1 1 0 106.465 5.05l-.708-.707a1 1 0 00-1.414 1.414l.707.707zm1.414 8.486l-.707.707a1 1 0 01-1.414-1.414l.707-.707a1 1 0 011.414 1.414zM4 11a1 1 0 100-2H3a1 1 0 000 2h1z" clipRule="evenodd" /></svg> }
function MoonIcon() { return <svg viewBox="0 0 20 20" fill="currentColor" className="w-4 h-4"><path d="M17.293 13.293A8 8 0 016.707 2.707a8.001 8.001 0 1010.586 10.586z" /></svg> }
  ...
  return (
    <div className="min-h-screen flex flex-col bg-ibm-gray10 dark:bg-ibm-gray100">
      ...
      <header className="surface border-b border-ibm-gray20 dark:border-ibm-gray80 no-print">
        <div className="max-w-screen-xl mx-auto px-4 lg:px-8 h-12 flex items-center gap-3">
          ...
          {totalFilled > 0 && (
            <span className="text-xs text-ibm-gray50 dark:text-ibm-gray30 hidden md:block">
              {totalFilled} campos completados
            </span>
          )}

          <div className="ml-auto">
            <button onClick={() => setDark(d => !d)} className="btn-ghost p-2" title={dark ? 'Modo claro' : 'Modo oscuro'}>
          ...
          {STEPS.map((s, i) => {
            ...
            return (
              <button key={s.id} onClick={() => i <= step && setStep(i)} disabled={i > step}
                className={`flex-1 flex flex-col sm:flex-row items-center gap-1 sm:gap-2
                            py-3 px-2 text-xs sm:text-sm font-medium border-b-2 transition-colors
                            ${active ? 'border-ibm-blue text-ibm-blue'
                              : done  ? 'border-transparent text-ibm-gray60 dark:text-ibm-gray30 hover:text-ibm-gray100 dark:hover:text-ibm-gray10 cursor-pointer'
                              : 'border-transparent text-ibm-gray50 cursor-not-allowed opacity-40'}`}
              >
                <span className={`w-5 h-5 rounded-full flex items-center justify-center text-xs flex-shrink-0
                                  ${active ? 'bg-ibm-blue text-white' : done ? 'bg-ibm-green text-white'
      ...
      <main className="flex-1 max-w-screen-xl mx-auto w-full px-4 lg:px-8 py-6 lg:py-8">
        <div className={step === 2 ? 'lg:grid lg:grid-cols-[260px_1fr] lg:gap-8' : 'max-w-2xl'}>
          ...
          {step === 1 && (
            <div className="step-enter">
              <SectionHeader title="Entorno físico" sub="Condiciones del site e inventario de equipos existentes" />
          ...
          {step === 2 && (
            <Questionnaire
              ...
              answers={answers} onChange={setAnswers}
            />
          )}

          {step === 3 && (
            <div className="step-enter">
              <SectionHeader title="Diagrama de arquitectura" sub="Generado automáticamente desde los datos completados" />
      ...
      <footer className="surface border-t border-ibm-gray20 dark:border-ibm-gray80 no-print">
        <div className="max-w-screen-xl mx-auto px-4 lg:px-8 h-14 flex items-center justify-between gap-4">
          ...
          <span className="text-xs text-ibm-gray50 hidden sm:block">Paso {step + 1} de {STEPS.length}</span>
          {step < STEPS.length - 1 ? (
            ...
            <button
              onClick={() => { setStep(0); setClient({}); setSite({}); setDomains(['redes', 'seguridad']); setAnswers({}) }}
